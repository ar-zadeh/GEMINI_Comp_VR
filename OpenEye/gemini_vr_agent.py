...
def _get_tools(executor, grounder, tracker):
    ...
    def track_object(object_description: str):
        ...
        # ground_object returns {'box_2d': [ymin, xmin, ymax, xmax], ...}
        # SAM 2 needs [ymin, xmin, ymax, xmax] (normalized) which matches our format?
        # ObjectTracker.track expects [ymin, xmin, ymax, xmax] normalized.
        ...
        # Cleanup temp frames? Maybe keep for debug.
        # shutil.rmtree(temp_dir) 
        
        return f"Tracking completed. Output video: {video_output}"

    return [start_bridge, move_relative, teleport, rotate_device, inspect_surroundings, locate_object, capture_video, track_object, finish_task, get_connection_status]
