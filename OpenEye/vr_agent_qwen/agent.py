...
class QwenAgent:
    ...
    def __init__(self):
        ...
        self.mss_capture_region: Optional[Dict[str, int]] = None
        self._init_headset_window_region()
    ...
    def _init_headset_window_region(self):
        """Find the 'Headset Window' position/size at startup for MSS screen capture."""
        region = self._find_window_region("Headset Window")
        if region:
            self.mss_capture_region = region
            self.logger.info(f"Headset window region found: {region}")
            print(
                "Headset Window region detected for MSS capture: "
                f"left={region['left']}, top={region['top']}, "
                f"width={region['width']}, height={region['height']}"
            )
        else:
            self.logger.warning(
                "Could not find 'Headset Window'. MSS capture will use monitor fallback."
            )

    def _find_window_region(self, window_title: str) -> Optional[Dict[str, int]]:
        """Resolve window bounds with Windows lookup first, then Linux fallbacks."""
        # WSL/native-linux with Windows apps: query real desktop windows via PowerShell.
        windows_region = self._find_window_region_windows(window_title)
        if windows_region:
            return windows_region

        # Primary path: wmctrl -lG
        try:
            proc = subprocess.run(
                ["wmctrl", "-lG"],
                capture_output=True,
            ...
            self.logger.debug(f"wmctrl window lookup failed: {e}")

        # Fallback path: xwininfo -name "Headset Window"
        try:
            proc = subprocess.run(
            ...
            self.logger.debug(f"xwininfo window lookup failed: {e}")
            return None

    def _find_window_region_windows(self, window_title: str) -> Optional[Dict[str, int]]:
        """Resolve window bounds via PowerShell + user32 GetWindowRect."""
        ps_script = rf"""
Add-Type @'
using System;
...
'@;
$p = Get-Process |
  Where-Object {{ $_.MainWindowTitle -and $_.MainWindowTitle -like "*{window_title}*" }} |
...
"""
        ...
        try:
            ...
            if proc.returncode != 0:
                self.logger.debug(f"PowerShell window lookup failed: {proc.stderr.strip()}")
                return None
            ...
            self.logger.debug(f"PowerShell window lookup exception: {e}")
            return None
